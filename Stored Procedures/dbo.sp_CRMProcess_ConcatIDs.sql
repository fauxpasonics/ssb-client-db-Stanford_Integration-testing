...
WHERE TM.[GUID] = z.[GUID] AND tm.[SourceSystem] = 'Stanford_TI'
...
FROM (SELECT CustomID1,[GUID], MAX(CAST(Primary_Flag as int)) Primary_Flag 
		FROM stg.tbl_CRMProcess_NonWinners WHERE [SourceSystem] = 'Stanford_TI'group by CustomID1,[GUID]) acct
...
UPDATE a
SET TI_Ids = ISNULL(LTRIM([b].[ConcatIDs1]),'')
